...
var isNarcissistic = function(num) {
	...
	var numDigits = numArr.length;
	for (digit of numArr) {
		sum += Math.pow(digit,numDigits); // or sum += digit ** numDigits
	}
	return sum === num;
};
...
var isNarcissisticAlt = function(num) {
	return num.toString().split('').map((x,i,arr) => x ** arr.length).reduce((a,b)=> a+b) === num;
}

//TESTS
isNarcissisticAlt(153);
isNarcissisticAlt(1652);
